library(readr)
library(dplyr)
main_data <- read_csv("INFOMELP_April 12, 2025_16.02.csv")
View(main_data)
...
filtered_main_data <- main_data %>% filter(!is.na(StartDate), Status !='Survey Preview', Exp_condition != '')
...
# All
qqnorm(final_data$false_memories)
qqline(final_data$false_memories)

# Condition: Control
qqnorm(control_data$false_memories)
qqline(control_data$false_memories)

# Condition: Chatbot w/o 
qqnorm(static_chatbot_data$false_memories)
qqline(static_chatbot_data$false_memories)

# Condition: Chatbot w/ feedback
qqnorm(chatbot_data$false_memories)
qqline(chatbot_data$false_memories)


#####========== Kruskal–Wallis test & Dunn’s Test ==========#####

# Kruskal–Wallis test
kruskal.test(false_memories ~ `Exp_condition`, data = final_data) # p-value = 0.000866
...
dunn.test(
  x = final_data$false_memories, g = final_data$`Exp_condition`,
  method = "BH"
)
# result: none-chatbot 具有顯著差異

#####========== Mixed Effect Model (Origianl) ==========#####
library(Matrix)
library(lme4)
library(MASS)
...
model_nb <- glm.nb(
  ...
  + `Personal Experience related to crime scene` + `Interest in crime scenes and investigations`
  + `Tendency to recommend the AI` + `Perceived AI trustworthy` + `Perceived AI empathy`
  + `AI Attitude` + Feedback,
  data = final_data
)
summary(model_nb)

## Check Residual Normality
resid_values_nb <- residuals(model_nb)
qqnorm(resid_values_nb)
qqline(resid_values_nb)
...
## Check Residual Normality
resid_values_pois <- residuals(model_pois)
qqnorm(resid_values_pois)
qqline(resid_values_pois)

## Check Poisson Model Dispersion
sum(residuals(model_pois, type = "pearson")^2) / df.residual(model_pois) # 0.5545151, underdispersion


# Quasi-Poisson: Since Poisson model underdispersion
# model_qpois <- glm(false_memories ~ Age + `Experience with AI` + `Experience with Chatbot` 
#                    + `Personal Experience related to crime scene` + `Interest in crime scenes and investigations`
#                    + `Tendency to recommend the AI` + `Perceived AI trustworthy` + `Perceived AI empathy`
#                    + `AI Attitude` + Feedback,
#                    data = final_data,
...
BIC(model_nb, model_pois)


#####========== Mixed Effect Model (New) Add interaction ==========#####
library(Matrix)
library(lme4)
library(MASS)
...
model_nb_interaction <- glm.nb(
  ...
  + `Tendency to recommend the AI` + `Perceived AI trustworthy` + `Perceived AI empathy`
  + `AI Attitude` + Feedback 
  + Feedback*`Experience with AI` + Feedback*`Experience with Chatbot` 
  + Feedback*`Tendency to recommend the AI` + Feedback*`Perceived AI trustworthy` 
  + Feedback*`Perceived AI empathy` + Feedback*`AI Attitude` ,
  data = final_data
)
summary(model_nb_interaction)

## Check Residual Normality
resid_values_nb_interaction <- residuals(model_nb_interaction)
qqnorm(resid_values_nb_interaction)
qqline(resid_values_nb_interaction)


# Poisson Model
model_pois_interaction <- glm(
  false_memories ~ Age + `Experience with AI` + `Experience with Chatbot` 
  + `Personal Experience related to crime scene` + `Interest in crime scenes and investigations`
  + `Tendency to recommend the AI` + `Perceived AI trustworthy` + `Perceived AI empathy`
  + `AI Attitude` + Feedback 
...
summary(model_pois_interaction)

## Check Residual Normality
resid_values_pois_interaction <- residuals(model_pois_interaction)
qqnorm(resid_values_pois_interaction)
qqline(resid_values_pois_interaction)

## Check Poisson Model Dispersion
sum(residuals(model_pois_interaction, type = "pearson")^2) / df.residual(model_pois_interaction) # 0.4172303, underdispersion


# Model Comparison
AIC(model_nb, model_pois, model_nb_interaction, model_pois_interaction)
BIC(model_nb, model_pois, model_nb_interaction, model_pois_interaction)



# Poisson Model -- Interaction: Experience with AI, Experience with Chatbot 
# Result: `Perceived AI trustworthy`, Feedback: p = 0.09
model_pois_interaction <- glm(
  false_memories ~ Age + `Experience with AI` + `Experience with Chatbot` 
  + `Personal Experience related to crime scene` + `Interest in crime scenes and investigations`
  + `Tendency to recommend the AI` + `Perceived AI trustworthy` + `Perceived AI empathy`
